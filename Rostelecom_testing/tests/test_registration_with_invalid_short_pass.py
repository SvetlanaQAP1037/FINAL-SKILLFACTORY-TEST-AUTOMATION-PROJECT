# Run test:
# python -m pytest -v --driver Chrome --driver-path C:/Chromedriver/chromedriver.exe test_registration_with_invalid_short_pass.py

from pages.reg_page import RegPage
from faker import Faker
from settings import rand_email

def test_registration_with_invalid_short_pass(web_browser):
    ...
    page.password.send_keys(fake_pass.password(length=5, special_chars=True, digits=True, upper_case=True, lower_case=True))
    page.confirm_password.click()

    # Делаем скриншот некорректно заполненного поля
    page.password.highlight_and_make_screenshot('screenshots\\short_pass.png')
